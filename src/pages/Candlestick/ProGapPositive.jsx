...
const ProGapPositive = () => {
    const [data, setData] = useState([]);
    const [isLoading, setIsLoading] = useState(false);

    useEffect(() => {
        const fetchProGapPositiveData = async () => {
            try {
                setIsLoading(true);
                const response = await apiClient.get('/patterns/pro-gap-positive/');
                console.log("responseData: ", response);
                setData(Array.isArray(response.data) ? response.data : []);
            } catch (error) {
                console.error('Failed to fetch pro gap positive data:', error);
                setData([]);
            } finally {
                setIsLoading(false);
            }
                ...
                <main className="container mx-auto px-4 py-8">
                    {isLoading ? (
                        ...
                        <div className="space-y-12">
                            <PatternSection
                                title="Pro Gap Positive Pattern"
                                stocks={data}
                                patternType="pro_gap_positive"
                                icon={
