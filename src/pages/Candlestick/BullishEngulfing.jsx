...
const BullishEngulfing = () => {
    ...
    useEffect(() => {
        const fetchBullishEngulfingData = async () => {
            try {
                setIsLoading(true);
                const response = await apiClient.get('/patterns/bullish-engulfing/');
                console.log("responseData: ", response);
                setData(Array.isArray(response.data) ? response.data : []);
            } catch (error) {
                console.error('Failed to fetch bullish engulfing data:', error);
                setData([]);
            } finally {
                setIsLoading(false);
            }
