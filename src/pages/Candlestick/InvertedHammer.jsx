...
const InvertedHammer = () => {
    ...
    useEffect(() => {
        const fetchInvertedHammerData = async () => {
            try {
                setIsLoading(true);
                const response = await apiClient.get('/patterns/inverted-hammer/');
                console.log("invertedHammerResponseData: ", response);
                setData(Array.isArray(response.data) ? response.data : []);
            } catch (error) {
                console.error('Failed to fetch inverted hammer data:', error);
                setData([]);
            } finally {
                setIsLoading(false);
            }
